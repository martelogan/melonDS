...
void OptionsMenu()
{
    Menu(" Back     € OK", Options.size(), OptionsButtonActions, OptionsDrawRow);
}
...
void FilesMenu()
{
    ...
    while (ROMPath.find(".nds", ROMPath.length() - 4) == string::npos)
    {
        ...
        Menu("ƒ Exit     ‚ Options      Back     € OK", Files.size(), FilesButtonActions, FilesDrawRow);
        if (ROMPath == "")
            return;
...
{
    Pause();
    Menu("€ OK", PauseMenuItems.size(), PauseButtonActions, PauseDrawRow);
}
